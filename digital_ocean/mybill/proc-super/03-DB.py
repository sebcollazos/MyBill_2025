...
select a.nit,
	...
	CASE
        ...
        WHEN (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 > 2 AND (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 <= 4  THEN 10
        WHEN (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 > 4 AND (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 <= 6  THEN 15
        WHEN (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 > 6 AND (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 <= 8  THEN 20
        WHEN (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4 > 8 AND (a.pts_razon_cte + a.pts_ciclo_op +a. pts_end_total + a.pts_renta_op)/4       THEN 30
    END cupo
from puntos a 
inner join caratula as b on b.nit = a.nit
inner join ciiu as c on c.codigo = a.codigo
...
nit_sql = "select * from scoring order by nit"
# Cursor object
cursor  = conn.cursor()
...
for row in rows:
    ...
    with open(nit_file + ".json", 'a') as outfile:
        ...
        str_01 = ',"razon":"' + str(row[1]) + '"'
        str_02 = ',"pyme":"' + str(row[2]) + '"'
        str_03 = ',"ind":"' + str(row[3]) + '"'
        str_04 = ',"ciiu":"' + str(row[4]) + '"'
        str_05 = ',"actividad":"' + str(row[5]) + '"'
        str_06 = ',"riesgo_actividad":"' + str(row[6]) + '"'
        str_07 = ',"direccion":"' + str(row[7]) + '"'
        str_08 = ',"ciudad":"' + str(row[8]) + '"'
        str_09 = ',"depto":"' + str(row[9]) + '"'
        str_10 = ',"pais":"' + str(row[10]) + '"'
        str_11 = ',"telefono":"' + str(row[11]) + '"'
        str_12 = ',"celular":"' + str(row[12]) + '"'
        str_13 = ',"email":"' + str(row[13]) + '"'
        str_14 = ',"scoring":' + str(row[14]) + ''
        str_15 = ',"cupo":' + str(row[15]) + ''
        strFinal = remove_control_characters(str_00 + str_01 + str_02 + str_03 + str_04 + str_05 + str_06 + str_07 + str_08 + str_09 + str_10 + str_11 + str_12 + str_13 + str_14 + str_15)
