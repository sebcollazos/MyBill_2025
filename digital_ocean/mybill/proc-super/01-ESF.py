...
with open(default_json) as json_file:
    ...
    for file in data['files']:
        ...
        for n_fila in range(n_rows) :
            ...
            s_nit = ESF.iloc[n_fila]['Nit'].astype('int32').astype(str)
            ...
            s_v1 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v3'][0]
            s_v3 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v4'][0]
            s_v4 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['tot_AC_CO'][0]
            s_tot_AC_CO = ESF.iloc[n_fila][ columna ].astype("int64")

            s_v2 = s_tot_AC_CO - (s_v1 + s_v3 + s_v4)
            s_v2 = s_v2.astype("int64")
            ...
            s_v5 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v6'][0]
            s_v6 = ESF.iloc[n_fila][ columna ].astype("int64")
            
            columna = template['v7'][0]
            s_v7 = 0
            if columna in ESF.columns :
                s_v7 = ESF.iloc[n_fila][ columna ].astype("int64")
            else:
                s_v7 = ESF.iloc[n_fila][ template['v7'][1] ].astype("int64") + ESF.iloc[n_fila][ template['v7'][2] ].astype("int64") 

            columna = template['v9'][0]
            s_v9 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['tot_AC_NCO'][0]
            s_tot_AC_NCO = ESF.iloc[n_fila][ columna ].astype("int64")

            s_v8 = s_tot_AC_NCO - (s_v5 + s_v6 + s_v7 + s_v9)
            s_v8 = s_v8.astype("int64")
            ...
            s_tot_ACTIVOS = ESF.iloc[n_fila][ columna ].astype("int64")
            ...
            s_v10 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v11'][0]
            s_v11 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v12'][0]
            s_v12 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v13'][0]
            s_v13 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v14'][0]
            s_v14 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['tot_PS_CO'][0]
            s_tot_PS_CO = ESF.iloc[n_fila][ columna ].astype("int64")

            s_v15 = s_tot_PS_CO - (s_v10 + s_v11 + s_v12 + s_v13 + s_v14)
            s_v15 = s_v15.astype("int64")
            ...
            s_v16 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v17'][0]
            s_v17 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v18'][0]
            s_v18 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v19'][0]
            s_v19 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['tot_PS_NCO'][0]
            s_tot_PS_NCO = ESF.iloc[n_fila][ columna ].astype("int64")

            s_v20 = s_tot_PS_NCO - (s_v16 + s_v17 + s_v18 + s_v19)
            s_v20 = s_v20.astype("int64")
            ...
            s_tot_PASIVOS = ESF.iloc[n_fila][ columna ].astype("int64")
            ...
            s_v21 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v22'][0]
            s_v22 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v23'][0]
            s_v23 = ESF.iloc[n_fila][ columna ].astype("int64")

            columna = template['v24'][0]
            s_v24 = ESF.iloc[n_fila][ columna ].astype("int64")

            s_v25 = 0

            columna = template['tot_PATRIMONIO'][0]
            s_tot_PATRIMONIO = ESF.iloc[n_fila][ columna ].astype("int64")

            s_v26 = s_tot_PATRIMONIO - (s_v21 + s_v22 + s_v23 + s_v24 + s_v25)
            s_v26 = s_v26.astype("int64")
            ...
            s_tot_PASIVO_PATRIMONIO = ESF.iloc[n_fila][ columna ].astype("int64")
